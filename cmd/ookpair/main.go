...
func Main(app *gopi.AppInstance, done chan<- struct{}) error {
	/*
		...
		if payload_on, err := hex.DecodeString("800000008EE8EE888EE8EE888EE8EEEE"); err != nil {
			return err
		} else if payload_off, err := hex.DecodeString("800000008EE8EE888EE8EE888EE8EEE8"); err != nil {
			return err
		} else {
			for i := 0; i < 5; i++ {
				if i > 0 {
				...
				fmt.Println("ON", i)
				if err := ener314rt.Send(payload_on, 4, sensors.MIHOME_MODE_CONTROL); err != nil {
					return err
				}
				time.Sleep(time.Second)
				fmt.Println("OFF", i)
				if err := ener314rt.Send(payload_off, 4, sensors.MIHOME_MODE_CONTROL); err != nil {
					return err
				}
			}
		}
